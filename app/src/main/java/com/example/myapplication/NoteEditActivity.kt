...
class NoteEditActivity : AppCompatActivity(), DialogInterface.OnClickListener {
    ...
    private lateinit var urgentRadioButton: RadioButton
    private lateinit var notUrgentRadioButton: RadioButton
    private var imageUri: String? = null
    private var urgencyStatus: Boolean = false
    ...
    private fun initializeViews() {
        ...
        radioGroup = findViewById(R.id.urgency_radio_group)
        radioGroup.check(radioGroup.getChildAt(0).id)
        urgentRadioButton = findViewById(R.id.urgent)
        notUrgentRadioButton = findViewById(R.id.not_urgent)
    ...
    private fun getExistingNote() {
        ...
        if (id >= 0) {
            note = noteDao?.loadAllByIds(id)?.firstOrNull()
            note?.let {
                val bitmap: Bitmap? = it.image?.let {
                    img -> ImageConverter.convertStringToBase64(img)
                }
                editTitle.setText(it.title)
                urgencyStatus = it.urgency
                if (urgencyStatus) {
    ...
    private fun saveNote() {
        ...
        imageUri?.let {
            imageString = ImageConverter.convertDrawableToString(imagePreview.drawable)
        }
        ...
        note?.apply {
            ...
            this.urgency = urgencyStatus
            noteDao?.update(this)
        } ?: noteDao?.insertAll(Note(title, message, imageString, longitude, latitude, urgencyStatus))
